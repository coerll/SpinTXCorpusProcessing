...
function blowChunks(&$arr) {
	...
	foreach ($arr as $line) {
		...
		elseif ($newFile == true) {
			...
			$fineN2 = $fineN2[0];
			$fineN2 = mb_substr($fineN2,0,5, "UTF-8");
...
while (!feof($fhIDs)){
	...
	$FileData[$i] = preg_split("/\t/", $temporary);
	$i++;
}
...
for ($n = 1; $n <= $i; $n++) {
	...
	$ArrayOf4s[mb_substr($Files4[$n], 0, 5)] = $ArrayOf4s[$Files4[$n]];
...
//					fwrite ();
					...
					//lowercase the word.
					$target = mb_convert_case($target, MB_CASE_LOWER, "UTF-8");
					...
					if ((mb_strpos($target, "'", 0, "UTF-8") === false and findEnglish($target, $tarE)) or $target == "'cause") {
					...
					elseif (mb_strpos($target, "'", 0, "UTF-8") !== false) {
						...
						$subTar2 = mb_substr($target, $apPos, mb_strlen($target, "UTF-8") - $apPos, "UTF-8");
...
foreach($Array5 as $value3) {
	...
	elseif (!isset($File6)) {
		...
		//opens the file
		$File6 = "Processing/Tagged/".$value3[12].".txt";
		...
		fwrite($fh6, "Word Number	Original Word	POS	Lemma	Punctuation	Starting Character	Ending Character	Speaker	Start Time	End Time	SRT Line	Language	File ID	Tense	Mood	Number	Person	Gender\n");
	...
	elseif ($File6 != "Processing/Tagged/".$value3[12].".txt") {
		fclose($fh6);
		$File6 = "Processing/Tagged/".$value3[12].".txt";
		$fh6 = fopen($File6, 'w') or die("can't open File6");
		$newFiles[] = $File6;
